...
// Class App
class App {
  // await fetchMovies promise then render moveies from HomePage
  static async run(movieLink , genreId=null) {
    let movies = await APIService.fetchMovies(movieLink,genreId);
    let movieGenres = await APIService.fetchFilterMovis()
    HomePage.renderMovies(movies, movieGenres);
  }
  // await search promise then render search from HomePage
  static async search(query) {
    const search = await APIService.search(query);
    HomePage.renderSearch(search);
  }
  // await filterMovieType promise then render filter from HomePage
  static async filterMovieType(){
    const movieGenres = await APIService.fetchFilterMovis();
    HomePage.renderFilter(movieGenres);
  }
  // await filterMovieType promise then render filter from HomePage
    static async allstaffs() {
    const actor = await APIService.fetchPopularActors();
    HomePage.renderstaff({ actor });
  }
}

// Class APIService 
class APIService {
  //The Basic link of API (The Root)
  static TMDB_BASE_URL = "https://api.themoviedb.org/3";
  // Search static function  
  static async search(query) {
    //Search using the user input 
  ...
  // fetch  *all movies for single actor* static function
  ...
  // fetch *all staff for single movie* static function
  ...
  // Fetch for *5 similar movie for one single movie* static function
  ...
  // Fetch for *trailer from YouTube site for one single movie* static function
  ...
  // Fetch all *all movies' genres* static function
  ...
  // static function for the path of fetching
...
// Class HomePage 
class HomePage {
  // get Container Div elements from HTML 
  static container = document.getElementById("container");
  static mainContainer = document.getElementById("mainContainer");

  //render Movies Lists (now_playing, filter)
  static renderMovies(movies, movieGenres) {
    ...
    movies.forEach(movie => {
      ...
      let movieType = [];
      movieGenres.forEach(type =>{
        for (let i = 0 ; i<movie.genresID.length ; i++){
          if (type.id == movie.genresID[i]){
            
            movieType.push(type.name)
          }  
        }
      })
      ...
      // Div element contain info displayed by by mouseover
      const hiddenDiv = document.createElement("div");
      hiddenDiv.className="hideDiv";
      const movieDetails = document.createElement("p");
      movieDetails.innerHTML = `Raiting: ${movie.vote_average} <br> Movie Genres: ${movieType}`;
  ...
  // render for search function in the navBar
  ...
  // render function for about in the navBar
  static renderAbout(){  
   this.mainContainer.innerHTML = "";
    this.container.innerHTML = "";
  ...
  // render for list of all actor 
  static renderstaff(actor) {

    this.mainContainer.innerHTML = "";
    this.container.innerHTML = "";

    const staffcontainer = actor.actor.reduce((acc, currentValue) => {
      return (acc = `${acc}
      <div class = "col">
...
class ActorsSection {
  static renderActors(data) {
    ...
    const deathData = actor.deathday ? `<p><b>Died:</b> ${actor.deathday}</p>` : "";
...
// Finaly The DOMContentLoaded Function 
